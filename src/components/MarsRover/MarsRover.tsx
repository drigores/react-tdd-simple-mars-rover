...
const MarsRover = ({commands, entryPoint= "0:0:N"}:MarsRoverProps)=> { 
  ...
  const [MIN_SIZE, MAX_SIZE] = ["0","9"];
   ...
   const shouldWrapArround = ({x, y, direction}:Coordinate) =>{
        if(direction == Direction.NORTH && parseInt(y) == 9) return true;
        if(direction == Direction.SOUTH && parseInt(y) == 0) return true;
        if(direction == Direction.WEST && parseInt(x) == 0) return true;
        if(direction == Direction.EAST && parseInt(x) == 9) return true;
        return false;
    }
    ...
    const isCommandAMove = (character: string)=>{ return character == "M"}
